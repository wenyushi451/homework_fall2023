...
class SoftActorCritic(nn.Module):
    ...
    def q_backup_strategy(self, next_qs: torch.Tensor) -> torch.Tensor:
        ...
        if self.target_critic_backup_type == "doubleq":
            next_qs = torch.flip(next_qs, [0])
        elif self.target_critic_backup_type == "min":
            next_qs = torch.min(next_qs, 0)
        elif self.target_critic_backup_type == "mean":
            next_qs = torch.mean(next_qs, 0)
        else:
    ...
    def update_critic(
        ...
        with torch.no_grad():
            ...
            target_values: torch.Tensor = reward + self.discount * next_qs * (1 - done.type(torch.int))
    ...
    def entropy(self, action_distribution: torch.distributions.Distribution):
        ...
        return -torch.mean(probs * log_probs)
    ...
    def actor_loss_reinforce(self, obs: torch.Tensor):
        ...
        with torch.no_grad():
            ...
            action = torch.concat([self.actor(obs).sample() for _ in range(self.num_actor_samples)]).reshape((-1, batch_size, self.action_dim))
            assert action.shape == (
        ...
        loss = torch.neg(torch.mean(advantage * log_probs))
    ...
    def update(
        ...
        for _ in range(self.num_critic_updates):
            critic_infos.append(self.update_critic(obs=observations, action=actions, reward=rewards,
